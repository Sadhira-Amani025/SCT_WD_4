// script.js - full to-do logic with smart validation for both fields
...
(function() {
    ...
    // ----- ADD TASK (FIXED - Reminds when task name entered but no date) -----
    function addTask() {
        const text = taskInput.value.trim();
        const hasDate = taskDateTime.value ? true : false;
        
        // CASE 1: No task name entered
        if (text === '') {
            if (hasDate) {
                ...
                setTimeout(() => {
                    taskInput.style.borderColor = '#374151';
                }, 3000);
                return;
            }
            alert('⚠️ Please write a task name before adding!');
            taskInput.focus();
            taskInput.style.borderColor = '#ef4444';
            setTimeout(() => {
                taskInput.style.borderColor = '#374151';
        ...
        // CASE 2: Task name entered but NO date entered - REMINDER
        if (text !== '' && !hasDate) {
            // Show reminder about missing date
            const confirmAdd = confirm('⚠️ You haven\'t set a due date for this task.\n\nClick "OK" to add without date, or "Cancel" to add a date.');
            if (!confirmAdd) {
                // User wants to add a date - focus on date picker
                taskDateTime.focus();
                taskDateTime.style.borderColor = '#fbbf24';
                setTimeout(() => {
                    taskDateTime.style.borderColor = '#374151';
                }, 3000);
                return;
            }
            // User confirmed - add without date
        }
        
        // CASE 3: Date is entered BUT not confirmed (calendar picker used)
        if (hasDate && isUsingCalendarPicker && !isDateConfirmed) {
